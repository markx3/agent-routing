...
| `scribe` | opus | medium | Renders a plan or report as a claude.ai artifact for approval, following the artifact-design skill |
...
`/mstack:orchestrate <issue | spec path | request>` runs the whole pipeline and confirms with you at five gates: the plan (published as an artifact), the diagnosis, the review breadth, the decision after each watcher report, and any reply a builder would post on the PR. Everything else proceeds without asking.
...
The `review format` setting picks the report shape. `brief`, the default, groups one-line findings by axis and ends with a verdict table. `walkthrough` follows `references/review-format-walkthrough.md`, modeled on CodeRabbit's PR reviews: a walkthrough with a changes table by layer and a review effort estimate, a pre-merge checks table, findings per file with a category and severity label, a proposed fix, and a prompt a coding agent can act on, trivial findings collapsed as nitpicks, and a verdict table per axis. It renders as a GitHub comment unchanged.

The spec and standards axes adapt Matt Pocock's `code-review` skill, including its Fowler smell baseline. The other axes and the P0-P3 scale adapt Every's `ce-code-review` from the compound-engineering plugin. The prose rules in `references/prose.md`, which every role and the orchestrator write by, condense pstack's `unslop` skill. The planner adapts ideas from pstack's `architect` and `figure-it-out`, superpowers' `brainstorming`, and Matt Pocock's `to-spec`, `codebase-design`, and `handoff`.
