# Security
Gate: on when the diff touches authentication, authorization, session handling, public endpoints, user input parsing, file paths from input, shell execution, secrets, or permissions. Adapted from Every's `ce-code-review`, security reviewer.
