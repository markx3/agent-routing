...
Subagents come from the `agent-routing` plugin and are addressed as `agent-routing:<name>` (`feature-planner`, `debug-planner`, `builder`, `reviewer`, `explorer`, `mechanic`). Subagents can spawn subagents up to three layers deep (`CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH`); planners use that for explorers, builders and reviewers do not delegate.
