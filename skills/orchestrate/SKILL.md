...
7. Review phase. G4, then spawn `reviewer` with: the PR or diff range, the spec, the axes directory, the breadth, the parallel cap, the sub-reviewer model, its handoff path, and the template. On request changes, increment the round counter and go to step 6 with the report. On approve, continue.
